...
contract BoringGenerativeNFT is IBoringGenerativeNFT, BoringMultipleNFT, BoringOwnable {
    ...
    function addTrait(string calldata name, ITrait trait) public override onlyOwner {
        ...
        _traits.push(trait);
        require(
            _traits[gene].setName(gene, name) == bytes4(
                keccak256("setName(uint8,string)")
            ), "Bad return");
    }
    ...
    function addTraitData(uint8 trait, bytes calldata data) public onlyOwner {
        ...
        require(
            _traits[trait].addData(trait, data) == bytes4(
                keccak256("addData(address,uint8,bytes)")
            ), "Bad return");
    }
    ...
    function _renderTrait(uint256 tokenId, uint256 traitCount, uint8 trait, uint8 gene) internal view returns (bytes memory) {
        return abi.encodePacked(
            traitCount > trait ? _traits[0].renderTrait(this, tokenId, trait, gene) : "", 
            traitCount > trait + 1 ? "," : ""
        );
    }
