...
interface ITrait {
    ...
    function renderTrait(IBoringGenerativeNFT nft, uint256 tokenId, uint8 trait, uint8 gene) external view returns (string memory output);
    function renderSVG(IBoringGenerativeNFT nft, uint256 tokenId, uint8 trait, uint8 gene) external view returns (string memory output);
}
