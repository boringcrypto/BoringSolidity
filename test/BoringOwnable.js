...
describe("BoringOwnable", function () {
  ...
  describe("Renounce Ownership", function () {
    it("Prevents non-owners from renouncement", async function () {
      await expect(this.ownable.as(this.bob).renounceOwnership()).to.be.revertedWith(
        "Ownable: caller is not the owner"
      )
    })

    it("Assigns owner to address zero", async function () {
      await expect(this.ownable.renounceOwnership()).to.emit(this.ownable.contract, "OwnershipTransferred").withArgs(this.alice.address, ADDRESS_ZERO)
  ...
  describe("Transfer Ownership", function () {
    it("Prevents non-owners from transferring", async function () {
      await expect(this.ownable.as(this.bob).transferOwnership(this.bob)).to.be.revertedWith(
        "Ownable: caller is not the owner"
      )
    })
  ...
  describe("Claim Ownership", function () {
    ...
    it("Prevents anybody but pending owner from claiming ownership", async function () {
      await expect(this.ownable.as(this.bob).claimOwnership()).to.be.revertedWith(
