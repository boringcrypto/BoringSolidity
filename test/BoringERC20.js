...
describe("BoringERC20", function () {
  ...
  it("Can send tokens with transfer", async function () {
    await expect(this.token.transfer(this.mock.address, 100)).to.emit(this.token, "Transfer").withArgs(this.alice.address, this.mock.address, 100)
    await expect(this.mock.safeTransfer(this.bob.address, 100)).to.emit(this.token, "Transfer").withArgs(this.mock.address, this.bob.address, 100)
  ...
  it("Can send tokens with transferFrom", async function () {
    ...
    await expect(this.mock.safeTransferFrom(this.alice.address, this.carol.address, 50)).to.emit(this.token, "Transfer").withArgs(this.alice.address, this.carol.address, 50)
