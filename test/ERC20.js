...
describe("ERC20", function () {
  ...
  describe("Transfer", function () {
    ...
    it("Fails transfering max tokens from alice to alice", async function () {
      const token = await this.MockERC20.deploy("115792089237316195423570985008687907853269984665640564039457584007913129639935")
      await token.deployed()
      await expect(token.transfer(this.alice.address, "115792089237316195423570985008687907853269984665640564039457584007913129639935")).to.be.revertedWith("ERC20: overflow detected")
    })
  ...
  describe("TransferFrom", function () {
    ...
    it("Fails transferingFrom max tokens from alice to alice", async function () {
      ...
      await expect(token.transferFrom(this.alice.address, this.alice.address, "115792089237316195423570985008687907853269984665640564039457584007913129639935")).to.be.revertedWith("ERC20: overflow detected")
    })
  })
