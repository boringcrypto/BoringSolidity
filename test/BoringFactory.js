...
describe("BoringFactory", function () {
    ...
    it("Can create clone", async function () {
        ...
        await expect(this.contract.deploy(this.master.address, initData)).to.emit(this.contract, "LogDeploy")
    })

    it("Reverts on masterContract address 0", async function () {
        let initData = await this.master.getInitData(1234)
        await expect(this.contract.deploy(ADDRESS_ZERO, initData)).to.revertedWith("BoringFactory: No masterContract")
    })
})
