...
describe("BoringMultipleNFT", async function () {
    ...
    describe("mint function", async function () {
        ...
        it("should mint the NFT and transfer the NFT from 0 to the to address", async function () {
            ...
            await expect(this.contract.mint(this.bob.address)).to.emit(this.contract, "Transfer").withArgs(ADDRESS_ZERO, this.bob.address, 1)
        })
    ...
    describe("transferFrom function", async function () {
        ...
        it("should throws if msg.sender is not the owner ", async function () {
            await expect(this.contract.transferFrom(this.bob.address, this.alice.address, 0)).to.be.revertedWith("Transfer not allowed")
        })
    ...
    describe("approve function", async function () {
        it("should throw if the msg.sender is not the owner of the NFT", async function () {
            await expect(this.contract.connect(this.alice).approve(this.carol.address, 0)).to.be.revertedWith("Not allowed")
        })

        it("should throw if the operator is unauthorized", async function () {
            await expect(this.contract.connect(this.carol).approve(this.alice.address, 1)).to.be.revertedWith("Not allowed")
        })
    ...
    describe("safeTransferFrom function", async function () {
        ...
        it("should transfer when the operator is authorized by the original owner of the NFT", async function () {
            ...
            await this.contract
                .connect(this.alice)
                .functions["safeTransferFrom(address,address,uint256)"](this.carol.address, this.bob.address, 0)
        })

        it("should transfer an nft from the owner to the receiver", async function () {
            const sendFromAliceToBob = await this.contract
                .connect(this.bob)
                .functions["safeTransferFrom(address,address,uint256)"](this.bob.address, this.alice.address, 0)
            assert.equal(Number(sendFromAliceToBob.value), 0)
        ...
        it("should call onERC721TokenReceived on the contract it was transferred to", async function() {
            ...
            await this.contract
                .connect(this.bob)
                .functions["safeTransferFrom(address,address,uint256)"](this.bob.address, this.receiver.address, 3)
        ...
        // Same test with operator dirk
        // Same tests on safeTransferFrom(address,address,uint256,bytes)

        // Create mock that returns the wrong value and test with both functions
    })
})
